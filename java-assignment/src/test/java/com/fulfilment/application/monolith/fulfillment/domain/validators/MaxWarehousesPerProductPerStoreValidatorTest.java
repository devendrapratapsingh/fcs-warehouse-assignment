...
class MaxWarehousesPerProductPerStoreValidatorTest {
  ...
  void shouldPass_whenNoWarehousesAssignedYet() {
    when(fulfillmentStore.countWarehousesForProductInStore(PRODUCT, STORE)).thenReturn(0L);
  ...
  void shouldThrow_whenMaxWarehousesReached() {
    when(fulfillmentStore.countWarehousesForProductInStore(PRODUCT, STORE)).thenReturn(2L);
